...
//    static List<Endpoint> createEndpoints(MadoManifest manifest) {
//        MadoConfiguration configuration = manifest.getConfiguration();
...
//                .addPayloadMimeType("text-html")
//        );
//        return Stream.of(wadoEndpoint, webViewer).filter(Optional::isPresent).map( Optional::get ).map(obj -> (Endpoint)obj).toList();
//    }
//}
